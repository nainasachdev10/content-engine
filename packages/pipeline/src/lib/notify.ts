...
export function notifyStatus(): { email: boolean; push: boolean; pushSubscribers: number } {
  ...
  return { email, push, pushSubscribers };
}

export async function notify(n: Notification): Promise<void> {
  const url = `${dashboardUrl()}${n.path}`;
  const results = await Promise.allSettled([sendEmail(n, url), sendPush(n, url)]);
